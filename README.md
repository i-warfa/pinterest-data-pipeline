...
- Create a DAG using `Airflow` and upload it a `AWS MWAA` environemnt to periodically trigger the Databricks notebook for batch processing.
...
- After the streaming data has been cleaned, write each stream to a Delta Table. 
...
- Step 3: Setup `IAM Authentication` on the `EC2` to produce messages to the `AWS MSK` cluster. This package is necessary to connect to `AWS MSK` clusters that require `IAM Authentication`.
  ...
  * Naviagte to the `kafka_installation_folder/libs` directory and install the `IAM Authentication` package:
...
- Step 5: Run the [Batch Data](./user_posting_emulation.py) script. This script sends data from the 3 tables to their corresponding Kafka topic. Data passing through the `AWS MSK` cluster should be falling into the `S3` bucket.
...
- Step 1: Read the `AWS Credentials` that have been uploaded to a Databrick delta table from within the Notebook and mount the `S3` bucket to Databricks.
...
- Step 2: Data cleaning. Performing necessary tranformations of the `pinterest_data` dataframe
...
- Step 3: Create a DAG using `Airflow` and upload it a `AWS MWAA` environemnt to periodically trigger the Databricks notebook for batch processing. See [Airflow DAG](./airflow/12f6b2c1ae4f_dag.py)
...
- Step 2: Configure the [REST API](#configure-a-rest-api-in-api-gateway) created in the previous steps to allow for `Kinesis Proxy integration. Under the streams resource create a new child resource with the Resource name {stream-name}. The congiguration should look like this:

![](./images/REST_API_kinesis_integration.png)

- Step 3: Run the [Stream Data](./user_posting_emulation_streaming.py) script. This script sends a `PUT` requests to the API, which adds one record at a time to the 3 `Kinesis` streams, one for each pinterest table.

- Step 4: Read in the data from `Kinesis` streams by creating a `streaming dataframe` for structured streaming events, rather than a static dataframe which is created for batch workloads. See
